...
export default function DashboardPage() {
  ...
  const successRate =
    totalApplications > 0
      ? Math.round((stats.Accepted / totalApplications) * 100)
      : 0;
      ...
      <div className="max-w-7xl mx-auto">
        ...
        >
          ...
          <div className="flex gap-3">
            ...
            <Link
              href="/dashboard/candidate/resume-analyzer"
              className="px-5 py-2.5 rounded-lg border-2 border-primary/20 hover:border-primary/50 bg-primary/5 text-primary font-semibold hover:bg-primary/10 transition-all flex items-center gap-2"
            >
              <svg
                className="w-4 h-4"
                fill="none"
            ...
            {applications.length === 0 ? (
              <div className="text-center py-20 px-6">
                ...
                <p className="text-foreground/60 mb-6 max-w-md mx-auto">
                  Start your job search journey by exploring opportunities and
...
function StatCard({ title, value, index }) {
  ...
  return (
    ...
    >
      {/* Hover gradient */}
      <div
        className={`absolute inset-0 ${cfg.bg} opacity-0 group-hover:opacity-100 transition-opacity duration-500`}
      />
      ...
      <div className="relative">
        ...
        <div className="flex items-center justify-between mb-4">
          <div
            className={`w-12 h-12 rounded-lg ${cfg.bg} border ${cfg.border} flex items-center justify-center ${cfg.color} group-hover:scale-110 transition-transform duration-300`}
          >
            {cfg.icon}
          </div>
        ...
        {/* Value */}
        <div
          className={`text-4xl font-bold ${cfg.color} mb-2 group-hover:scale-105 transition-transform duration-300`}
        >
          {value}
        </div>
      ...
      {/* Corner accent */}
      <div
        className={`absolute bottom-0 right-0 w-20 h-20 ${cfg.bg} rounded-tl-full opacity-0 group-hover:opacity-100 transition-opacity duration-300`}
      />
    </motion.div>
  );
}
...
function StatusPill({ status }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold ${cfg.bg} ${cfg.text} border ${cfg.border}`}
    >
      {cfg.icon}
      {status}
    </span>
